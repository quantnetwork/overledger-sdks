...
BlockSearchAPI_autoExecuteSearchBlockRequest(apiClient_t *apiClient, char * Authorization , char * blockId , prepare_search_schema_t * prepare_search_schema )
{
    ...
    // create the path
    long sizeOfPath = strlen("/v2/autoexecution/search/block/{blockId}")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/v2/autoexecution/search/block/{blockId}");
    ...
    auto_execute_search_block_response_schema_t *elementToReturn = auto_execute_search_block_response_schema_parseFromJSON(BlockSearchAPIlocalVarJSON);
