...
#include <stdio.h>
#include <ctype.h>
#include "RetrieveSupplyInformationForAQRCTokenAPI.h"
...
RetrieveSupplyInformationForAQRCTokenAPI_readQRC721SmartContractCurrentSupply(apiClient_t *apiClient, char * Authorization , char * flowType , prepare_request_t * prepare_request )
{
    ...
    // create the path
    long sizeOfPath = strlen("/v2/tokenise/tokens/{flowType}/current-supply")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/v2/tokenise/tokens/{flowType}/current-supply");
    ...
    cJSON *RetrieveSupplyInformationForAQRCTokenAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    token_read_qrc721_response_t *elementToReturn = token_read_qrc721_response_parseFromJSON(RetrieveSupplyInformationForAQRCTokenAPIlocalVarJSON);
    cJSON_Delete(RetrieveSupplyInformationForAQRCTokenAPIlocalVarJSON);
