  ...
  class FungibleTokenResponseDTO
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::FungibleTokenResponseDTO` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::FungibleTokenResponseDTO`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      contract_type_validator = EnumAttributeValidator.new('String', ["ERC20", "QRC20"])
    ...
    def contract_type=(contract_type)
      validator = EnumAttributeValidator.new('String', ["ERC20", "QRC20"])
    ...
    def hash
      [documentation_url, unit, decimal_places, functions, contract_type, location, smart_contract_id].hash
    end
