  ...
  class PaymentSchema
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::PaymentSchema` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::PaymentSchema`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if @amount.to_s.length > 100
      return false if @amount.to_s.length < 0
      return false if @amount !~ Regexp.new(/^[A-Za-z0-9. ]{1,100}/)
      return false if @unit.nil?
      return false if @unit.to_s.length > 5
      return false if @unit.to_s.length < 0
      return false if @unit !~ Regexp.new(/^[A-Za-z0-9 ]{1,5}/)
      true
    end
