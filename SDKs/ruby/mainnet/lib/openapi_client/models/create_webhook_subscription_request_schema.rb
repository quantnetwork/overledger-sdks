  ...
  class CreateWebhookSubscriptionRequestSchema
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::CreateWebhookSubscriptionRequestSchema` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::CreateWebhookSubscriptionRequestSchema`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@ids.nil? && @ids.length > 36
        invalid_properties.push('invalid value for "ids", number of items must be less than or equal to 36.')
      end

      if !@ids.nil? && @ids.length < 0
        invalid_properties.push('invalid value for "ids", number of items must be greater than or equal to 0.')
      end
    ...
    def valid?
      ...
      return false if !@ids.nil? && @ids.length > 36
      return false if !@ids.nil? && @ids.length < 0
    ...
    def ids=(ids)
      if !ids.nil? && ids.length > 36
        fail ArgumentError, 'invalid value for "ids", number of items must be less than or equal to 36.'
      end

      if !ids.nil? && ids.length < 0
        fail ArgumentError, 'invalid value for "ids", number of items must be greater than or equal to 0.'
      end
