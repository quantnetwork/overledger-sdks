  ...
  class Burn
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::Burn` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::Burn`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@metadata.nil? && @metadata.to_s.length > 150
        invalid_properties.push('invalid value for "metadata", the character length must be smaller than or equal to 150.')
      end
      ...
      if !@metadata.nil? && @metadata !~ pattern
        invalid_properties.push("invalid value for \"metadata\", must conform to the pattern #{pattern}.")
      end
    ...
    def valid?
      ...
      return false if !@metadata.nil? && @metadata.to_s.length > 150
      return false if !@metadata.nil? && @metadata.to_s.length < 0
      return false if !@metadata.nil? && @metadata !~ Regexp.new(/^[\S\s]{1,150}$/)
    ...
    def metadata=(metadata)
      if !metadata.nil? && metadata.to_s.length > 150
        fail ArgumentError, 'invalid value for "metadata", the character length must be smaller than or equal to 150.'
      end
      ...
      pattern = Regexp.new(/^[\S\s]{1,150}$/)
      if !metadata.nil? && metadata !~ pattern
        fail ArgumentError, "invalid value for \"metadata\", must conform to the pattern #{pattern}."
      end
    ...
    def hash
      [token_name, amount, unit, metadata, token_id].hash
    end
