  ...
  class Parameter
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::Parameter` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::Parameter`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@key.nil? && @key.to_s.length > 20
        invalid_properties.push('invalid value for "key", the character length must be smaller than or equal to 20.')
      end
    ...
    def valid?
      ...
      return false if !@key.nil? && @key.to_s.length > 20
      return false if !@key.nil? && @key.to_s.length < 0
      return false if !@key.nil? && @key !~ Regexp.new(/^[A-Za-z0-9]{1,20}$/)
      true
    end
    ...
    def key=(key)
      if !key.nil? && key.to_s.length > 20
        fail ArgumentError, 'invalid value for "key", the character length must be smaller than or equal to 20.'
      end
