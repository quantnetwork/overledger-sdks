  ...
  class AddressMonitoringRequestSchema
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::AddressMonitoringRequestSchema` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::AddressMonitoringRequestSchema`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@address_id.nil? && @address_id.to_s.length > 200
        invalid_properties.push('invalid value for "address_id", the character length must be smaller than or equal to 200.')
      end
    ...
    def address_id=(address_id)
      if !address_id.nil? && address_id.to_s.length > 200
        fail ArgumentError, 'invalid value for "address_id", the character length must be smaller than or equal to 200.'
      end
