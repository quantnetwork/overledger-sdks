  ...
  class CreateAQRC721TransferTransactionApi
    ...
    def prepare_transaction_request_with_http_info(authorization, prepare_payment_transaction_request_schema, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: CreateAQRC721TransferTransactionApi.prepare_transaction_request ...'
      end
      # verify the required parameter 'authorization' is set
      if @api_client.config.client_side_validation && authorization.nil?
        fail ArgumentError, "Missing the required parameter 'authorization' when calling CreateAQRC721TransferTransactionApi.prepare_transaction_request"
      end
      if @api_client.config.client_side_validation && authorization.to_s.length > 5010
        fail ArgumentError, 'invalid value for "authorization" when calling CreateAQRC721TransferTransactionApi.prepare_transaction_request, the character length must be smaller than or equal to 5010.'
      end

      if @api_client.config.client_side_validation && authorization.to_s.length < 0
        fail ArgumentError, 'invalid value for "authorization" when calling CreateAQRC721TransferTransactionApi.prepare_transaction_request, the character length must be great than or equal to 0.'
      end

      pattern = Regexp.new(/^Bearer [A-Za-z0-9\-_. ]{1,5010}$/)
      if @api_client.config.client_side_validation && authorization !~ pattern
        fail ArgumentError, "invalid value for 'authorization' when calling CreateAQRC721TransferTransactionApi.prepare_transaction_request, must conform to the pattern #{pattern}."
      end
      ...
      if @api_client.config.client_side_validation && prepare_payment_transaction_request_schema.nil?
        fail ArgumentError, "Missing the required parameter 'prepare_payment_transaction_request_schema' when calling CreateAQRC721TransferTransactionApi.prepare_transaction_request"
      end
      # resource path
      local_var_path = '/v2/tokenise/preparation/transaction/qrc721/transfer'
      ...
      new_options = opts.merge(
        :operation => :"CreateAQRC721TransferTransactionApi.prepare_transaction_request",
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: CreateAQRC721TransferTransactionApi#prepare_transaction_request\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
