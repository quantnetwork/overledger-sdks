  ...
  class SmartContractSearchFunctionSchema
    ...
    def self.openapi_types
      {
        ...
        :'output_parameters' => :'Array<SmartContractOutputParameterSchema>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::SmartContractSearchFunctionSchema` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::SmartContractSearchFunctionSchema`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if !@name.nil? && @name !~ Regexp.new(/^[A-Za-z0-9]{1,20}$/)
    ...
    def hash
      [name, input_parameters, output_parameters].hash
    end
