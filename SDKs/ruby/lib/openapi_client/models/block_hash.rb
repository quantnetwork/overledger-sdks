  ...
  class BlockHash
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::BlockHash` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::BlockHash`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if !@type.nil? && @type !~ Regexp.new(/^[a-zA-Z0-9_]{1,50}$/)
      return false if !@value.nil? && @value.to_s.length > 150
      return false if !@value.nil? && @value.to_s.length < 0
      return false if !@value.nil? && @value !~ Regexp.new(/^[a-zA-Z0-9]{1,150}$/)
      true
    end
