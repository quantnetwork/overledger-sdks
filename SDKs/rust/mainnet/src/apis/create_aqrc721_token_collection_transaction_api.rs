...
/// struct for typed errors of method [`prepare_collect_request`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum PrepareCollectRequestError {
...
pub async fn prepare_collect_request(configuration: &configuration::Configuration, authorization: &str, prepare_payment_transaction_request_schema: crate::models::PreparePaymentTransactionRequestSchema) -> Result<crate::models::PrepareTransactionResponse, Error<PrepareCollectRequestError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/v2/tokenise/preparation/transaction/qrc721/collect", local_var_configuration.base_path);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<PrepareCollectRequestError> = serde_json::from_str(&local_var_content).ok();
