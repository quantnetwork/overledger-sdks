...
/// struct for typed errors of method [`subscribe_collect_event`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum SubscribeCollectEventError {
...
pub async fn subscribe_collect_event(configuration: &configuration::Configuration, authorization: &str, body: serde_json::Value) -> Result<crate::models::EventSubscriptionResponse, Error<SubscribeCollectEventError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/v2/tokenise/tokens/subscription/qrc721/collect", local_var_configuration.base_path);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<SubscribeCollectEventError> = serde_json::from_str(&local_var_content).ok();
