...
/// struct for typed errors of method [`prepare_secondary_owner_request`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum PrepareSecondaryOwnerRequestError {
...
pub async fn prepare_secondary_owner_request(configuration: &configuration::Configuration, authorization: &str, prepare_add_secondary_account_request_schema: crate::models::PrepareAddSecondaryAccountRequestSchema) -> Result<crate::models::PrepareTransactionResponse, Error<PrepareSecondaryOwnerRequestError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/v2/preparation/secondaryaccountowner", local_var_configuration.base_path);
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::POST, local_var_uri_str.as_str());
    ...
    local_var_req_builder = local_var_req_builder.json(&prepare_add_secondary_account_request_schema);
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<PrepareSecondaryOwnerRequestError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
