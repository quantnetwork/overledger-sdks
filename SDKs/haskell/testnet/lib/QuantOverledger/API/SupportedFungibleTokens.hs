...
module QuantOverledger.API.SupportedFungibleTokens where
...
-- 
getTokens
  :: QuantOverledgerRequest GetTokens MimeNoContent FungibleTokenResponseDTO MimeJSON
getTokens =
  _mkRequest "GET" ["/v2/tokens/fungible"]

data GetTokens  
instance HasOptionalParam GetTokens Location2 where
  applyOptionalParam req (Location2 xs) =
    req `addQuery` toQuery ("location", Just xs)
instance HasOptionalParam GetTokens ContractType where
  applyOptionalParam req (ContractType xs) =
    req `addQuery` toQuery ("contractType", Just xs)
instance HasOptionalParam GetTokens Unit where
  applyOptionalParam req (Unit xs) =
    req `addQuery` toQuery ("unit", Just xs)
-- | @application/json@
instance Produces GetTokens MimeJSON
