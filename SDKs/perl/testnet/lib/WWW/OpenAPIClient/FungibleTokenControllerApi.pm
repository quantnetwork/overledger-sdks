...
#
package WWW::OpenAPIClient::FungibleTokenControllerApi;
...
sub get_token {
    ...
    unless (exists $args{'technology'}) {
      croak("Missing the required parameter 'technology' when calling get_token");
    }

    # verify the required parameter 'network' is set
    unless (exists $args{'network'}) {
      croak("Missing the required parameter 'network' when calling get_token");
    }

    # verify the required parameter 'unit' is set
    unless (exists $args{'unit'}) {
      croak("Missing the required parameter 'unit' when calling get_token");
    }

    # parse inputs
    my $_resource_path = '/v2/token/fungible';
    ...
    my $_response_object = $self->{api_client}->deserialize('Erc20DTO', $response);
