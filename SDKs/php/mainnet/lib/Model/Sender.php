...
class Sender implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public function setAccountId($account_id)
    {
        if (!is_null($account_id) && (mb_strlen($account_id) > 500)) {
            throw new \InvalidArgumentException('invalid length for $account_id when calling Sender., must be smaller than or equal to 500.');
        }
        if (!is_null($account_id) && (mb_strlen($account_id) < 0)) {
            throw new \InvalidArgumentException('invalid length for $account_id when calling Sender., must be bigger than or equal to 0.');
        }
        if (!is_null($account_id) && (!preg_match("/^[a-zA-Z0-9:,\/.=\\-\\s]{1,500}/", $account_id))) {
            throw new \InvalidArgumentException("invalid value for $account_id when calling Sender., must conform to the pattern /^[a-zA-Z0-9:,\/.=\\-\\s]{1,500}/.");
        }
