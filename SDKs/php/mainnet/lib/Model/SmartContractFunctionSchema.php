...
/**
 * SmartContractFunctionSchema Class Doc Comment
 *
 * @category Class
 * @package  OpenAPI\Client
 * @author   OpenAPI Generator team
 ...
 */
class SmartContractFunctionSchema implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'SmartContractFunctionSchema';
    ...
    public function setName($name)
    {
        if (!is_null($name) && (mb_strlen($name) > 50)) {
            throw new \InvalidArgumentException('invalid length for $name when calling SmartContractFunctionSchema., must be smaller than or equal to 50.');
        }
        if (!is_null($name) && (mb_strlen($name) < 0)) {
            throw new \InvalidArgumentException('invalid length for $name when calling SmartContractFunctionSchema., must be bigger than or equal to 0.');
        }
        if (!is_null($name) && (!preg_match("/^[a-zA-Z0-9,_ ]{1,50}$/", $name))) {
            throw new \InvalidArgumentException("invalid value for $name when calling SmartContractFunctionSchema., must conform to the pattern /^[a-zA-Z0-9,_ ]{1,50}$/.");
        }
    ...
    public function setInputParameters($input_parameters)
    {
        ...
        if (!is_null($input_parameters) && (count($input_parameters) > 20)) {
            throw new \InvalidArgumentException('invalid value for $input_parameters when calling SmartContractFunctionSchema., number of items must be less than or equal to 20.');
        }
        if (!is_null($input_parameters) && (count($input_parameters) < 0)) {
            throw new \InvalidArgumentException('invalid length for $input_parameters when calling SmartContractFunctionSchema., number of items must be greater than or equal to 0.');
        }
