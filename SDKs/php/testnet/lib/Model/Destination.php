...
class Destination implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public function setDestinationId($destination_id)
    {
        if (!is_null($destination_id) && (mb_strlen($destination_id) > 66)) {
            throw new \InvalidArgumentException('invalid length for $destination_id when calling Destination., must be smaller than or equal to 66.');
        }
        ...
        if (!is_null($destination_id) && (!preg_match("/^[A-Za-z0-9:]{1,66}/", $destination_id))) {
            throw new \InvalidArgumentException("invalid value for $destination_id when calling Destination., must conform to the pattern /^[A-Za-z0-9:]{1,66}/.");
        }
