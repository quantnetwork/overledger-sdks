...
class Mandate implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public function setPayeeId($payee_id)
    {
        if (!is_null($payee_id) && (mb_strlen($payee_id) > 500)) {
            throw new \InvalidArgumentException('invalid length for $payee_id when calling Mandate., must be smaller than or equal to 500.');
        }
        if (!is_null($payee_id) && (mb_strlen($payee_id) < 0)) {
            throw new \InvalidArgumentException('invalid length for $payee_id when calling Mandate., must be bigger than or equal to 0.');
        }
